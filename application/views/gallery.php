          ...
          <ul id="mycarouse3" class="jcarousel-skin-tango">
            <?php if ($galleries->num_rows() > 0): ?>
              <?php foreach ($galleries->result_array() as $galleries_item): ?>
                <li>
                  <a href="<?php echo base_url() . $galleries_item['gallery_image'] ?>" data-rel="prettyPhoto[mg_2]" title="<?php echo $galleries_item['title'] ?>"><img src="<?php echo base_url() . $galleries_item['gallery_image'] ?>" width="100" height="100" alt=""><span></span>
                  </a>
                </li>
              <?php endforeach; ?>
            <?php else : ?>
